<template>
  <div class="home-content" @touchmove="$root.handleTouch">
    <div class="user-data">
      <div v-if="device === 'pc'" class="icon_promotion-img" @click="promotionDialogShow = true">
        <img src="./../../../../static/images/icon_share.png" alt="" />
      </div>
      <template v-if="!!myUserInfo.icon">
        <div class="info-box">
          <div :class="{ 'service-icon': myUserInfo.isCustomerService }"></div>
          <el-image :src="noIconShow(myUserInfo, 'user')" />
        </div>
      </template>
      ...
      <div>
        ...
        <div class="group-edit-btn" @click="$router.push({ path: '/EditUser' })">
          <img src="./../../../../static/images/edit.png" alt="" />
          <span class="group-edit-text">编辑头像昵称</span>
        </div>
      </div>
    </div>
    <template v-if="device === 'mobile'">
      <div class="setting-button" v-for="(item, index) in settingData" :key="index" :class="{
        mt10: ['相片和影片', '关于嗨聊', '提醒'].includes(item.name),
      }">
      ...
      <div class="setting-button bor_mt10">
        <router-link :to="'/DeleteAccount'">
          <div class="setting-button-left">
            <img src="./../../../../static/images/x-octagon.png" alt="" />
            <span class="red-text">删除帐号</span>
          </div>
          <div class="setting-button-right">
            <img src="./../../../../static/images/next.png" alt="" />
          </div>
    ...
    <template v-else>
      <div class="setting-button" v-for="(item, index) in settingData" :key="index" :class="[
        ...
        { bor_mt10: ['提醒','密码管理','关于嗨聊'].includes(item.name)},
    ...
    <el-dialog :visible.sync="promotionDialogShow" class="el-promotion-dialog" width="80%" :show-close="false"
      :modal="false" center>
      <div class="qrcode-box">
        <vue-qr ref="Qrcode" :correctLevel="3" :autoColor="false" colorDark="#333333" :text="promoteCodeConfig.text"
          :logoSrc="promoteCodeConfig.logo" :size="1000" :margin="0" :logoCornerRadius="0" :logoMargin="1"></vue-qr>
      </div>
      <div class="promote-box-text">快来注册并下载嗨聊吧！</div>
      <div class="promote-box-text2">
        <span>{{ promoteCodeConfig.text }}</span>
      </div>
      <span slot="footer" class="dialog-footer">
...
export default {
    ...
    return {
      settingData: [
...

        {
          ...
          pcIcon: require("./../../../../static/images/shield.png"),
          path: "/PasswordMange",
        },
        {
          name: "封锁名单",
        ...
        // },
        {
          ...
          pcIcon: require("./../../../../static/images/pc/info.png"),
          path: "/About",
        },
      ],
      promoteCodeConfig: {
  ...
  created() {
    ...
    this.promoteCodeConfig.text = `${location.origin
  ...
  methods: {
    getService() {
      const service = {
        name: "调查记录",
        icon: require("./../../../../static/images/dcgl.png"),
        pcIcon: require("./../../../../static/images/phone_icon/dcgl.png"),
        path: "/Investigation",
      }
      if (this.myUserInfo.isCustomerService) {
        this.settingData.push(service);
      }
    },
